...
const BloodPage = () => {
  const [isDeleteBloodDialogOpen, setIsDeleteBloodDialogOpen] =
  useState(false); //Mangae the state of the dialog box
  const [blood, setblood] = useState<BloodDetails | null>(null); //Store the selected Resignation details null if user wants to create one
  const [formState, setFormState] = useState<FormState>("create"); //Manage the state  ["display", "create", "edit"]
  const { data: bloods, isLoading, isError } = useFetchBloods(); //Tanstack method
        ...
        <div className="tables flex w-full flex-col overflow-clip rounded-[9px]">
          ...
          {bloods?.map((item: BloodDetails, index) => {
            return (
              <div
                key={index + 1}
                className={`cell-1 flex w-full cursor-pointer flex-row items-center gap-2 px-3 py-2 text-zinc-700 ${blood?.id === item.id ? "bg-gray-100 text-white" : index % 2 === 0 ? "bg-white" : "bg-slate-50"} hover:bg-slate-100 active:bg-slate-200`}
                onClick={(e) => {
                  e.stopPropagation();
                  if (blood?.id === item.id) return;
              ...
              >
                ...
                <div className="flex min-w-[120px] flex-row gap-2 text-start text-sm font-medium">
                  <ButtonSm
                    className={`${formState === "edit" && blood?.id === item.id ? "bg-blue-500 text-white hover:bg-blue-500 hover:text-black active:bg-blue-600" : "bg-white"}`}
                    state="outline"
                    text="Edit"
                    onClick={(e) => {
                      e.stopPropagation();
